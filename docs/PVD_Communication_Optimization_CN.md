...
下面的 shadow 试验仅覆盖一个真实模型请求；全部 logits 的数值偏差、
长期反复刷新和跨机故障注入尚未验证，故 opt-in 仍不应设为默认。

### 真实 Qwen 请求的逐层 shadow 数值对照

提交 `543ae8821` 增加只用于验收的 `attention_impl=triton_shadow`：
在同一个 bank read lease 内先以 Triton 写正式输出，再以现有
online 实现写独立、已计费的 shadow 输出；CUDA 完成后逐元素
比较，容差 `atol=0.025, rtol=0.002`。失配时隔离 workspace，
保留指针表与相关资源，不继续使用未经验证的结果。故障注入
实际证明正常路径预算归零、故意篡改输出会隔离、容量不足
预先拒绝、完成状态不确定时资源不提前释放。一个测试曾只因
错误消息文本与断言不一致失败，已在 `4dd80c6e8` 修正并重跑通过。

隔离 D:30003/GPU1 使用真实 Qwen2.5-7B、同一 1,923-token
固定 Prompt 完整生成 8 token；输入与最终输出 SHA-256 与上述
A/B 相同。D 日志中共有 **196 次成功比较**，分别是
`decode_tokens=0…6` 的 7 次前向，每次 28 层；最大绝对差
`0.00390625`，无失配或隔离。该请求耗时 73.91 s，包含双算，
**不能**用于 Triton 性能比较。它把验证从合成张量扩展到了
真实模型在这些步骤实际消费的 Q、Prompt bank 和 D 生成 KV；
仍只覆盖这一模型、这一 Prompt/长度和 7 次注意力前向，
不证明所有 logits、长时间压力或其他模型的数值等价。
验证后隔离 D 恢复 online tile64，原有 D:30001 始终未动。

### 同输入 4 客户端 A/B（小样本）

提交 `98c8c387f` 为负载脚本增加 `--replay-seed`：同一 seed 的
每个 client 仍有独立 Prompt，但跨次运行保持相同字节和 SHA-256。
以 `matched4_20260926a`、55 次句子重复、每请求实际 516-token
Prompt、8-token 贪心输出，在同一 D 代码 `64910ffe1`、同一
P/V/Gateway 和 rank-packed 通信下，只切换 D 的 `attention_impl`：

| 4 客户端总墙钟 | 第 1 轮 | 第 2 轮 | 完成率 |
| --- | ---: | ---: | ---: |
| triton_grouped tile=64 | 57.73 s | 57.76 s | 4×8/4×8 |
| online tile=64 | 74.22 s | 64.97 s | 4×8/4×8 |

四个客户端的输入哈希在两种实现、四轮运行中逐一相同；
四个最终输出哈希也逐一相同。该 A/B 比之前“相近长度但不同输入”
的四客户端测试严格，但每模式只有两轮，且先 Triton 后 online，
不能排除共享节点负载、重启及服务排队造成的波动，不应宣传成
稳定的并发加速比。两 V rank 在每种实现结束后均为零在途、零
UNKNOWN、未隔离。试验后隔离 D:30003 留在保守的 online tile64；
原有 D:30001 始终未变。
